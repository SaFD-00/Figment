...
def downscale_to_png(data: bytes, max_side: int) -> bytes:
    ...
    small/correctly-sized uploads are not needlessly re-encoded. Used to cap local edit/reference
    inputs to a sane working resolution (see genspec.LOCAL_MAX_SIDE)."""
    img = Image.open(io.BytesIO(data))
    img = ImageOps.exif_transpose(img)
