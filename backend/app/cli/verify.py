...
def _build_cases(pid: str, pre: Prereqs, s: Samples) -> list[VerifyCase]:
    ...
    return [
        # ── LOCAL image/video (ComfyUI) — the H100 photoreal stack, per mode ──
        img_local("chroma-hd / txt2img", "txt2img", ["comfy", "model:chroma-hd"],
                  lambda: GenSpec(mode=Mode.txt2img, model="chroma-hd",
                                  prompt="a photorealistic red fox in a snowy forest, soft light",
                                  width=768, height=768)),
        img_local("chroma-hd / img2img", "img2img", ["comfy", "model:chroma-hd", "net"],
                  lambda: GenSpec(mode=Mode.img2img, model="chroma-hd",
                                  prompt="the same scene as an oil painting", denoise=0.6,
                                  source_asset=s.source)),
        img_local("lustify / txt2img", "txt2img", ["comfy", "model:lustify"],
                  lambda: GenSpec(mode=Mode.txt2img, model="lustify",
                                  prompt="a fast photorealistic studio portrait, soft light",
                                  negative_prompt="blurry", width=768, height=768)),
        img_local("flux-fill / inpaint", "inpaint", ["comfy", "model:flux-fill", "net"],
                  lambda: GenSpec(mode=Mode.inpaint, model="flux-fill",
                                  prompt="a blooming flower",
                                  source_asset=s.source, mask_asset=s.mask)),
        img_local("sdxl-inpaint / inpaint", "inpaint", ["comfy", "model:sdxl-inpaint", "net"],
                  lambda: GenSpec(mode=Mode.inpaint, model="sdxl-inpaint",
                                  prompt="a blooming flower", negative_prompt="blurry",
                                  source_asset=s.source, mask_asset=s.mask)),
        img_local("qwen-edit-aio / edit", "edit", ["comfy", "model:qwen-edit-aio", "net"],
                  lambda: GenSpec(mode=Mode.edit, model="qwen-edit-aio",
                                  prompt="make it look like winter with falling snow",
                                  source_asset=s.source)),
        img_local("qwen-edit-aio / edit (mask→inpaint)", "edit", ["comfy", "model:qwen-edit-aio", "net"],
                  lambda: GenSpec(mode=Mode.edit, model="qwen-edit-aio",
                                  prompt="replace the masked area with a blooming flower",
                                  source_asset=s.source, mask_asset=s.mask)),
        img_local("kontext / edit (reference)", "edit", ["comfy", "model:kontext", "net"],
                  lambda: GenSpec(mode=Mode.edit, model="kontext",
                                  prompt="apply the look of the reference to this image",
                                  source_asset=s.source,
                                  reference_images=[ReferenceImage(asset=a) for a in (s.ref1,) if a])),
        img_local("redux / reference (style)", "reference", ["comfy", "model:redux", "net"],
                  lambda: GenSpec(mode=Mode.reference, model="redux",
                                  prompt="a portrait in the style of this reference",
                                  reference_images=[ReferenceImage(asset=a) for a in (s.ref1,) if a])),
        img_local("ip-adapter / reference (identity)", "reference", ["comfy", "model:ip-adapter", "net"],
                  lambda: GenSpec(mode=Mode.reference, model="ip-adapter",
                                  prompt="a portrait keeping this person's identity",
                                  negative_prompt="blurry",
                                  reference_images=[ReferenceImage(asset=a) for a in (s.ref1,) if a])),
        img_local("lustify / controlnet", "controlnet",
                  ["comfy", "model:lustify", "cnet:canny", "controlnet-nodes", "net"],
                  lambda: GenSpec(mode=Mode.controlnet, model="lustify",
                                  prompt="a neon cyberpunk street, same composition",
                                  negative_prompt="blurry", controlnet_type="canny",
                                  source_asset=s.source)),
        img_local("wan22-ti2v / video", "video", ["comfy", "model:wan22-ti2v"],
                  lambda: GenSpec(mode=Mode.video, model="wan22-ti2v",
                                  prompt="a red fox trotting through falling snow, cinematic",
                                  width=704, height=704, video_frames=25)),
        img_local("wan22-i2v / video (image→video)", "video", ["comfy", "model:wan22-i2v", "net"],
                  lambda: GenSpec(mode=Mode.video, model="wan22-i2v",
