...
Figment unifies two heritages into one product: the **FigGen** scientific-figure pipeline (cloud, structured → editable SVG/PPTX) and the **ImgGen** local image studio (ComfyUI/Ollama). Both run behind **one engine interface** (`engines/base.py`): cloud (OpenRouter) models now also produce plain **raster images** for the normal modes — interchangeable with local — while structured figures are an explicit opt-in (`Mode.figure`); local models run ComfyUI. It's a Next.js UI over a single FastAPI backend. The user **no longer picks a generation mode** — the **chat LLM auto-routes** each request to the right `Mode` (asking a short clarifying question when the intent is ambiguous). The image **model** per mode is still UI-picked and applied as the override once the mode is routed — never in config.
...
- **Chat & prompt-enhance** are separate from the queue. `llm/routing.py:resolve_chat` powers **both** `routers/chat.py` and `routers/prompt.py` (`POST /prompt/enhance`): a cloud LLM (with a configured key) streams from **OpenRouter**, a local LLM from its **Ollama** tag, otherwise the default Ollama model — independently of the image model. The lineup is **vision-capable only** (local `qwen3-vl-local` + the cloud multimodal LLMs, all `vision=True`), so enhance can always ground the rewrite in an uploaded edit/reference image on either route. `llm/handoff.py` extracts a `GenSpec` from the conversation. The chat system prompt is also the **mode router**: it picks `GenSpec.mode` from the prompt and any attached image, and asks one short clarifying question (emitting no GENSPEC) when the intent is ambiguous. The home composer feeds the first prompt + uploads into this same path; `chat.py` notes the attachments to the vision LLM and, after routing, injects the uploaded asset ids into the routed spec (`source_asset` for image→image, `reference_images` for style/figure), re-validating so a mismatch degrades to a `genspec_error` rather than a 500.
...
### Frontend wiring
`frontend/lib/api.ts` / `sse.ts` hit a hardcoded `/api` base; `frontend/next.config.ts` rewrites `/api/*` → `http://127.0.0.1:8000/*` (backend host is hardcoded, no `NEXT_PUBLIC_API_URL`). The model picker is **backend-sourced** (`GET /api/models/all` → `lib/models.ts` store), grouped Local/Cloud by `engine`; nothing is hardcoded in the picker. Image/video models are still remembered **per mode** (store `selectedByMode` over all `GEN_MODES` incl. `video` and `figure`; `getImageModelForMode(mode)`) — applied as the override **after** the LLM routes the mode (editor chat `startSpec`, reference panel, toolbar). The **home composer** (`PromptBox`) has no mode tabs and no per-mode image pill: just a prompt, an optional multi-image dropzone, and the **LLM pill** (the router). Submitting creates a project, uploads attachments as `reference` assets, stashes `{prompt, attachments}` in the editor store's `pendingStart`, and navigates — **without creating a job**. `ChatPanel` consumes `pendingStart` once (guarded by a `useRef` latch + `messagesLoaded`/empty-conversation check for StrictMode safety) to auto-send the first chat turn; a confident first-turn route auto-runs the job, an ambiguous one asks in chat. `pendingStart` is intentionally **excluded from `reset()`** so the editor-page mount `reset()` can't wipe the handoff. The canvas is **react-konva**, dynamically imported `ssr:false` (Konva touches `window`); `next.config.ts` aliases the `canvas` module to `false`. Region-redraw and text-edit resolve their model through the per-mode pick (defaulting via `DEFAULT_BY_MODE` to `flux-fill` for inpaint and `qwen-edit-aio` for edit).
