...
모드를 직접 고를 필요는 없습니다 — 프롬프트(와 첨부 이미지)를 보고 **채팅 LLM이 알맞은 모드로 자동 라우팅**하며,
의도가 애매하면 캔버스 대화창에서 어떻게 진행할지 **되묻습니다**. 처음 입력한 프롬프트는 에디터 **대화창의 첫
메시지**로 이어집니다.
...
프롬프트만 입력하면(필요하면 이미지 첨부) **LLM이 알아서 Generate / Edit / Reference / Figure로 라우팅**합니다 —
아래 기능들은 별도 탭이 아니라 의도에 따라 자동으로 선택됩니다.
...
- 클라우드 이미지 모델은 일반 모드에서는 **래스터 이미지**를 생성하고(로컬과 상호 교체 가능), **`figure` 모드**에서만
  **FigGen 파이프라인**으로 구조화된 FigureSpec → 편집 가능한 SVG/PPTX를 산출합니다.

---

## 모델 — UI에서 직접 선택

**모드는 LLM이 라우팅**하므로 사용자는 모델만 고릅니다. 홈 프롬프트 박스에는 라우팅을 담당하는 **채팅/플래너 LLM
pill**만 두고, 이미지 모델은 LLM이 모드를 정한 뒤 **모드별 선택값이 자동 적용**됩니다(에디터 채팅·레퍼런스 패널·툴바의
인라인 pill picker에서 모드별로 기억). Local / Cloud로 그룹화되어 표시되며, **목록은 백엔드 `/models`에서 동적으로**
받아옵니다(프론트 하드코딩 없음).
...
LLM은 **Ollama**, 클라우드 LLM은 **OpenRouter**에서 스트리밍). 클라우드 이미지 모델은 일반 모드에서 **래스터 이미지**를
(로컬과 상호 교체 가능), **`figure` 모드**에서는 **FigGen 파이프라인**(SVG/PPTX)을 산출하고, 로컬 이미지 모델은
**ComfyUI**로 라우팅됩니다. API 키가 없으면 클라우드 옵션이 picker에서 비활성화되고, 앱은 로컬/mock으로 폴백해 오프라인으로 동작합니다.
...
| edit (지시 편집 · 레퍼런스 이미지로 피사체/얼굴) | `qwen-edit-aio` | Qwen-Image-Edit Rapid AIO |
| reference (스타일, 멀티 레퍼런스) | `redux` | FLUX Redux |
| **video** (text+image→video) | `wan22-ti2v` | Wan 2.2 TI2V-5B (t2v·i2v 모두 커버) |
| 채팅/플래너 LLM (멀티모달, prompt-enhance 이미지 입력) | `qwen3-vl-local` | Huihui Qwen3-VL 8B abliterated (uncensored) |

> 기능당 한 모델로 **정리(consolidate)**했습니다. 얼굴/피사체 정체성은 별도 모델 대신 `edit`(레퍼런스 이미지) ·
> `reference`로 처리합니다(consent-gated). 모든 가중치는 이미 디스크에 있어 **추가 다운로드가 필요 없습니다**.
...
backend/app/     FastAPI 호스트: routers (chat·jobs·projects·assets·uploads·models)
  engines/       엔진 디스패치(base.GenerationEngine): local-comfy · cloud-image(래스터) · figure(SVG/PPTX) · ollama
...
- **생성 작업**은 오케스트레이터 큐(`orchestrator/queue.py`)의 단일 워커가 처리합니다. 모델을 resolve한 뒤
  `_select_engine`이 **GenerationEngine**(`engines/base.py`)을 고르고 공통 `_persist`로 저장합니다: 로컬이면
  **ComfyUI**(`comfy/builder.py`가 그래프 구성 → `/prompt` + `/ws`, 영상은 animated webp), 클라우드 일반 모드면
  **래스터 이미지**(OpenRouter), 클라우드 `figure` 모드면 **FigGen 파이프라인**(FigureSpec → SVG/PPTX)으로 라우팅됩니다.
...
합법적인 **성인 합의** NSFW 생성만 전제로 합니다. 얼굴/피사체 작업(`edit` + 레퍼런스 이미지)은
